...
#include<arm_neon.h>
 ...
 namespace ISP {
    ...
    void Utils::multi(T* C, const T* A, const T* B) {
        ...
        ISP_ASSERT(2 == C->dimensions());
        ISP_ASSERT(2 == B->dimensions());
        ISP_ASSERT(2 == A->dimensions());
    ...
    void Utils::add(Tensor* C, const Tensor* A, const Tensor* B) {
        ISP_ASSERT(NULL != C);
        ISP_ASSERT(NULL != B);
        ISP_ASSERT(NULL != A);

        ISP_ASSERT(A->size() == C->size());
        auto height = A->length(0);
        auto width = A->length(1);
        int bOffset = 0;
        if (B->dimensions() == A->dimensions()) {
            bOffset = B->stride(0);
            ISP_ASSERT(B->length(1) == A->length(1));
            ISP_ASSERT(B->length(0) == A->length(0));
        }
        else {
            bOffset = 0;
            ISP_ASSERT(B->length(0) == A->length(1));
        }
    ...
    void Utils::dot(Tensor* C, const Tensor* A, const Tensor* B) {
        ISP_ASSERT(NULL != C);
        ISP_ASSERT(NULL != B);
        ISP_ASSERT(NULL != A);
        ISP_ASSERT(2 == C->dimensions());
        ISP_ASSERT(2 == B->dimensions());
        ISP_ASSERT(2 == A->dimensions());
        ISP_ASSERT(A->shape() == B->shape());
        ISP_ASSERT(A->shape() == C->shape());
    ...
    void Utils::invert(Tensor* dst, const Tensor* src) {
        ISP_ASSERT(2 == src->buffer().dimensions);
        const int N0 = src->buffer().dim[0].extent;
        ISP_ASSERT(N0 == src->buffer().dim[1].extent);
        ...
        for (i = 0; i < N0; ++i) {
            ...
            if (*(tempData + i * N0 + i) == 0) {
                ISP_PRINT("This matrix have no inverse!\n");
                return;
            }
    ...
    void Utils::transpose(Tensor* dst, const Tensor* src) {
    ...
    void Utils::print(const Tensor* C, const char* head) {
        auto c = C->host<float>();
        ...
        for (int y = 0; y < h; ++y) {
            for (int x = 0; x < w; ++x) {
                ISP_PRINT("%.7f\t", c[x + y * stride]);
            }
            ISP_PRINT("\n");
        }
    }

    void Utils::mulPerLine(Tensor* C, const Tensor* A, const Tensor* Line) {
        auto c = C->host<float>();
        auto a = A->host<float>();
        ...
        ISP_ASSERT(Line->buffer().dim[1].extent >= h);
        ISP_ASSERT(A->buffer().dim[0].extent == h);
        ISP_ASSERT(A->buffer().dim[1].extent == w);
        ISP_ASSERT(Line->buffer().dim[0].extent == 1);
    ...
    void Utils::divPerLine(Tensor* C, const Tensor* A, const Tensor* Line) {
        auto c = C->host<float>();
        auto a = A->host<float>();
        auto l = Line->host<float>();
        auto w = C->buffer().dim[1].extent;
    ...
    std::shared_ptr<Tensor> Utils::polyMulti(std::shared_ptr<Tensor> A, std::shared_ptr<Tensor> B) {
        ISP_ASSERT(A->buffer().dim[0].extent == 1);
        ISP_ASSERT(B->buffer().dim[0].extent == 1);
    ...
    float Utils::matDet(const Tensor* A) {
        ISP_ASSERT(2 == A->buffer().dimensions);
        const int n0 = A->buffer().dim[0].extent;
        ISP_ASSERT(n0 == A->buffer().dim[1].extent);
